...
def prep_df(df, target, cutoff = 3):
    ''' takes in a df and target. 
    drops nulls, duplicates, and removes outliers SD > 3'''
    df = df.dropna()
    df = df.drop_duplicates()
    df['zscore'] = stats.zscore(df[target])
    df = df[(np.abs(stats.zscore(df[target])) < cutoff)]
    return df
